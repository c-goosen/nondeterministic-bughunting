...
### Q1b — Infrastructure, deployment & operational controls

Goal: enrich `## 1. System context` (the "where it runs" prose), add
infrastructure-level rows to `## 3. Entry points & trust boundaries`, and
capture the operational controls that will move `likelihood` and `impact`
down in Q3. The application code rarely shows where privilege and exposure
*actually* live — the network edge, the IAM model, and the deploy pipeline do.
Ask these explicitly; do not infer them from the app code alone.

**Ground answers in infrastructure-as-code first.** Before asking, Glob the
target for `Dockerfile*`, `docker-compose*.y*ml`, `*.tf` / `*.tfvars`, `k8s/`
or `*.yaml` manifests, `.github/workflows/`, `Procfile`, `serverless.yml`,
`helm/`, `ansible/`, `cloudformation*`. Summarize what you find back to the
owner ("I see a Terraform module provisioning an RDS instance and an ALB; the
service runs on ECS Fargate") and ask them to confirm or correct. Where IaC
confirms an answer, tag `[Code-verified]`; where the owner asserts something
the IaC doesn't show, tag `[Owner-states]` and add it to `## 6. Open
questions`.

Work through these domains one at a time. For the enum-style ones (hosting
model, network exposure), `AskUserQuestion` gives the owner clean choices;
keep the open-ended ones conversational. Each domain notes where its answer
lands.

| # | Ask | Lands in / affects |
|---|---|---|
| Hosting & runtime | "Where does this run — managed cloud (which provider/service), self-managed k8s, serverless, VMs, bare metal, on-prem? Managed or self-operated?" | section 1 prose; sets which `actor`s can reach which surfaces |
| Network exposure | "What's reachable from the public internet vs internal-only? Is there a load balancer, WAF, CDN, API gateway, or VPN in front? What controls egress?" | new section 3 rows (edge as a trust boundary); infra `controls` that lower likelihood |
| Identity & access (IAM) | "How do services authenticate to each other and to cloud resources — service accounts, IAM roles, mTLS? Is least-privilege enforced, or are roles broad?" | section 3 trust boundaries; elevation-of-privilege threats in section 4 |
| Secrets management | "Where do credentials, API keys, and signing keys live — a vault/KMS, the orchestrator's secret store, env vars, or committed to the repo? Are they rotated?" | section 2 asset + sensitivity; `controls`; flags a secrets-in-repo threat (cross-check with a secret scan) |
| Data stores & encryption | "What datastores hold the section 2 assets? Encryption at rest and in transit? Managed or self-run? Where does PII physically live (region/tenancy)?" | section 2 sensitivity; `controls`; data-disclosure threats |
| Build & supply chain | "Walk me through deploy: who can push to prod, is the pipeline gated/reviewed, are artifacts and images signed, are dependencies and base images pinned?" | `supply_chain` actor threats in section 4; section 8 mitigations |
| Runtime isolation | "Do workloads run as non-root, in containers, with seccomp/AppArmor or a sandbox? Is there hard multi-tenant isolation, or shared trust inside the cluster?" | `controls` that lower the *impact* of an RCE/escape threat |
| Detection & response | "If someone exploited this, would you see it — centralized logs, alerting, audit trail of who did what? Who responds, and how fast?" | repudiation/DoS `controls`; informs likelihood that an attack is caught early |
| Availability & recovery | "Rate limiting or autoscaling at the edge? Backups, and have restores been tested? What's the blast radius if one node/region fails?" | DoS threat `impact`/`likelihood`; section 5 if DoS is accepted risk |

After the walk, **reconcile against section 3**: every infrastructure trust
boundary the owner named (edge → app, app → datastore, CI → prod, tenant →
tenant) should appear as a row in section 3 or be consciously folded into an
existing one. Infrastructure controls named here are not threats themselves —
they are the `controls` column and the section 8 mitigations you will fill in
Q3. Record any "we think X but aren't sure" answer as a section 6 open
question to verify against the IaC.
...
Include **infrastructure-layer** mitigations surfaced in Q1b where they close
a class more cheaply than app changes: WAF/edge rate limiting (DoS), least-
privilege IAM and network segmentation (lateral movement / EoP), a vault +
rotation (secret exposure), signed artifacts and pinned base images (supply
chain), non-root + seccomp/sandbox (RCE blast radius). Tag each section 8 row
implicitly by effort; infra controls are often `S`/`M` and close a `class`
outright.
